...
def analysis(features, params):
    ...
    ### Standaradize
    data.trainx, data.testx, scaler = sbitools.standardize(data.trainx, secondary=data.testx, log_transform=True)
    np.save("testy", data.testy)
    np.save("trainy", data.trainy)
...
def diagnostics_fiducial(data, posterior, scaler):
    ...
    argsf.datapath = 'z10-N0001/zheng07-fid/'
    argsf.fiducial = 1
    features, params = dataloaders.hod_ells(argsf)
