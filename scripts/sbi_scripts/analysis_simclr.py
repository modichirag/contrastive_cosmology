...
#####
#############
def analysis_simclr(features, params):
    ...
    device = 'cpu'
    nlatents = 32
    nembeddings = nlatents *2
    model = SimCLR(nlatents=nlatents, nembeddings=nembeddings, nlayers=2)
    if device == 'cuda': model.cuda()

    #setup dataloader
    shotnoise = False
    trainx = data.trainx.reshape(-1, data.nsim, data.nfeatures) # reshape for simclr
    train_loader = PS_loader(trainx, hybrid=0.5, shotnoise_amp=shotnoise, device=device)

    #setup simclr model
    try:
        if retrain: raise Exception
        model = torch.load(savepath+'model.pb')
    except:
        #setup loss function and training
        batch_size = 256
        lr = 1e-4
        epochs = 20000
        wdecay = 1e-3
        scheduler = None #lambda opt: optim.lr_scheduler.ExponentialLR(opt, gamma=0.95)
        info_nce_loss = Info_nce_loss(batch_size, n_views, device=device)
        criterion = nn.CrossEntropyLoss()
        losses, optimizer = simclr.train(model, train_loader,
                                         criterion=criterion, ssl_loss=info_nce_loss,
                                         lr=lr, batch_size=batch_size, epochs=epochs,
                                         scheduler=scheduler, weight_decay=wdecay)
    ...
    return data, model, train_loader, scaler
...
data, model, train_loader, scaler = analysis_simclr(features, params)
