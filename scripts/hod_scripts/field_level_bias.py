...
from nbodykit.lab import FFTPower, ProjectedFFTPower, ArrayMesh, BigFileMesh
import matplotlib
matplotlib.use('Agg')
...
def get_linear_mesh(i_lhc):
    linmesh = BigFileMesh(path=f"/mnt/ceph/users/cmodi/fastpm-shivam/WHITE_NOISE_LH_QUIJOTE/N{nc}_bigfile/{i_lhc}/linear/", dataset="LinearDensityK")
...
for i_lhc in range(args.id0, args.id1):
    ...
    for i_hod in range(args.nhod): 
        ...
        # sample HOD
        iseed = args.seed+i_hod+23*wrank
        ...
        if wrank == 0 : print(f"in rank {comm.rank}, thetahod : ", theta_hod)
        #hod = Galaxies.hodGalaxies(halos, theta_hod, seed=0, hod_model=m_hod)
        if hodmodel is None:
            np.random.seed(i_lhc*999+iseed)
    ...
    if comm.rank == 0:
        ...
        #np.save(save_dir + "power_eft", np.array(pseft).real)
